...
while (isNaN(money)) {
  alert('Введенные данные не являются числом! Попробуйте еще раз');
  money = prompt('Ваш месячный доход?', 125000)
}
...
while (isNaN(amount)) {
  alert('Введенные данные не являются числом! Попробуйте еще раз');
  amount = prompt('Во сколько обойдуться обязательные статьи расходов?', 24000)
}
...
let extraMoney = prompt(`Перечислите возможный доход за ваши дополнительные работы: ${profit}?`, 12800)
while (isNaN(extraMoney)) {
  alert('Введенные данные не являются числом! Попробуйте еще раз');
  extraMoney = prompt(`Перечислите возможный доход за ваши дополнительные работы: ${profit}?`, 12800)
}
